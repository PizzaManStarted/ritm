use egui::{
    include_image, style::WidgetVisuals, vec2, Align, Atom, AtomExt, AtomLayout, Button, CentralPanel, Color32, Context, Frame, Grid, Id, Image, ImageButton, ImageSource, IntoAtoms, Label, Layout, Margin, Modal, RichText, ScrollArea, Separator, SidePanel, Stroke, Ui, Vec2, Widget
};
...
fn icon(app: &mut App, ui: &mut Ui, text: impl Into<String>, source: ImageSource) {
    AtomLayout::new((
        Image::new(source)
            .tint(Color32::BLACK)
            .atom_size(Vec2::splat(50.0)),
        RichText::new(text).font(Font::default_medium())
    ).into_atoms()).gap(25.0).show(ui);
}
...
fn code(app: &mut App, ui: &mut Ui) {
    ...
    ui.add(text("The easiest way to create and share a turing machine is by using code !"));
...
fn controls(app: &mut App, ui: &mut Ui) {
    ...
    ui.add(text("The user can control how the turing machine run at the center of the controls section :"));
    ...
    ui.add(text("[Idle] when tapes and turing machine are at their initial state."));
    ui.add(text("[Accepted] when accept is the current state of the machine."));
...
fn graph(app: &mut App, ui: &mut Ui) {
    ...
    ui.add(text("It make easy to visualise how a turing machine work and troubleshoot any problem !"));
    
    ui.add(text("3 states will always be displayed and unremovable :"));
    ui.add(text("(i) : The initial state, where any machine start."));
    ui.add(text("(a) : The accept state, which mean the input is accepted when the machine reach it."));
    ui.add(text("(r) : The reject state, which is a way to cleanly stop the machine"));

    icon(app, ui, "Convert the graph into code", include_image!("../../../assets/icon/code.svg"));
    icon(app, ui, "You can use your mouse/finger to move, zoom and unzoom the graph around (On computer you need to use ctrl and scroll to zoom)qs", include_image!("../../../assets/icon/zoom.svg"));
